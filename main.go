...
func main() {
	...
	error.Check(err)
	connectionString = fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=disable", *postgresHostname, *postgresPort, *postgresUser, strings.TrimSpace(string(pass)), *postgresDb)
